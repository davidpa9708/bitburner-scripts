...
export function getMaxThreads(ns, script, host) {
  return Math.floor((ns.getServerMaxRam(host) - ns.getServerUsedRam(host)) / ns.getScriptRam(script));
}
...
export function divideServersV2(servers, times, getValue) {
  ...
  for (const server of sortedServers) {
    const i = _.minBy(_.range(times), (i2) => resultValue[i2]);
...
export function getSchedule(tasks, getTime) {
  ...
  return { tasks: adjusted, maxTime: maxTime - min2 - WAIT_TIME };
}
