...
export async function main(ns) {
  ...
  ns.disableLog("sleep");
  daemon(ns, false);
  const wait = async () => ns.sleep(waitDuration * 1e3);
  while (true) {
    const nodeCount = ns.hacknet.numNodes();
    const balance = ns.getServerMoneyAvailable("home");
    let bestNode = 0;
    let bestUpgradePath = "level";
    ...
    if (balance > upgradeCosts[bestUpgradePath](bestNode)) {
      ...
      if (result) {
        ns.print(`upgraded node ${bestNode} ${bestUpgradePath} to ${upgradeLevel}`);
        continue;
      }
      ns.print(`Error, cannot upgraded node ${bestNode} ${bestUpgradePath} to ${upgradeLevel}`);
      await wait();
    } else {
      ns.print(`not enough balance to upgraded node ${bestNode} ${bestUpgradePath} to ${upgradeLevel}`);
      await wait();
    }
    await ns.sleep(100);
  }
